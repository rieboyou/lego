...
importOption.textContent = "从文件导入颜色";
...
function addWaterMark(pdf, isHighQuality) {
    for (let i = 0; i < pdf.internal.getNumberOfPages(); i++) {
        ...
        pdf.text(
            ...
            "Generated by www.wanzhanjimu.com"
        );
		pdf.text(pdf.internal.pageSize.height * 0.55,pdf.internal.pageSize.height * 0.35,"@wanzhuan",-30);
        pdf.text(pdf.internal.pageSize.height * 0.06, pdf.internal.pageSize.height * 0.05, VERSION_NUMBER);
    }
}
    ...
    runStep4(async () => {
        ...
        for (var i = 0; i < totalPlates; i++) {
            ...
            if ((i + 1) % (isHighQuality ? 20 : 50) === 0) {
                ...
                pdf.save(`积木艺术画拼砌步骤图-Part-${numParts}.pdf`);
                numParts++;
                pdf = new jsPDF({
                    orientation: titlePageCanvas.width < titlePageCanvas.height ? "p" : "l",
                    unit: "mm",
        ...
        addWaterMark(pdf, isHighQuality);
        pdf.save(numParts > 1 ? `积木艺术画拼砌步骤图-Part-${numParts}.pdf` : "积木艺术画拼砌步骤图.pdf");
...
function handleInputImage(e, dontClearDepth, dontLog) {
    ...
    reader.onload = function (event) {
        ...
        document.getElementById("input-image-selector").innerHTML = "上传新图片";
