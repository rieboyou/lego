...
function generateInstructionTitlePage(
    ...
    ctx.fillStyle = "#000000";
    ctx.font = `${scalingFactor * 1.8}px Arial`;
    ctx.fillText("积木艺术画拼砌步骤图", pictureWidth * 0.75, pictureHeight * 0.28);
    ctx.font = `${scalingFactor}px Arial`;
    ctx.fillText(
        `像素点: ${width} x ${pixelArray.length / (4 * width)}`,
        pictureWidth * 0.75,
        pictureHeight * 0.36
    );
	ctx.font = `${scalingFactor / 2}px Arial`;
	ctx.fillText("颗粒色号匹配的是“乐高积木”的色号。", pictureWidth * 1.25, pictureHeight * 0.36);
...
function generateInstructionPage(
    ...
    ctx.font = `${scalingFactor}px Arial`;
    ctx.beginPath();
    ctx.fillText(`第 ${plateNumber} 块底板`, pictureWidth * 0.75, pictureHeight * 0.2 - scalingFactor);
    ctx.stroke();
